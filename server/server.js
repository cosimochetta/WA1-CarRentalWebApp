...
app.get('/api/vehicles', (req, res) => {
    vehicleDao.getVehicles(req.query.category, req.query.brand)
        .then((vehicles) => {
            res.json(vehicles);
        })
        .catch((err) => {
...
app.post('/api/rent', (req, res) => {
    const rent = req.body;
    if (!rent || !rent.user || !rent.starting_day || !rent.end_day || !rent.cost || !rent.km
        || !rent.driver_age || !rent.extra_driver || rent.extra_insurance === null || !rent.category) {
        res.status(400).end();
    } else {
        const user = req.user && req.user.user;
        if (rent.user != user)
        ...
        Promise.all([rentDao.countPastRents(user),
        rentDao.getAvailableVehicles(rent.category, rent.starting_day, rent.end_day),
        vehicleDao.getVehicles(rent.category, null)])
            .then((values) => {
                let [userRentCount, availableVehicles, vehicles] = values;
                rentDao.createRent(rent, userRentCount.count, availableVehicles, vehicles.length)
                    .then((id) => {
